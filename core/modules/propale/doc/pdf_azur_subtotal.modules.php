...
class pdf_azur_subtotal extends ModelePDFPropales
{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					$localtax2_type=$object->lines[$i]->localtax2_type;
						...
						if(property_exists($object, 'remise_percent')){
					...
					if (!empty($object->lines[$i]->TTotal_tva))
					{
						...
						{
							$this->tva[$vatrate] = array_key_exists($vatrate, $this->tva) ? $this->tva[$vatrate] + $tvaligne : $tvaligne;
						}
					}
					else {
						// standard
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if (!getDolGlobalString('PROPALE_PDF_HIDE_PAYMENTTERMCOND') && ($object->cond_reglement_code || $object->cond_reglement))
		{
			$pdf->SetFont('','B', $default_font_size - 2);
			$pdf->SetXY($this->marge_gauche, $posy);
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if($object->statut==0 && (getDolGlobalString('PROPALE_DRAFT_WATERMARK')) )
		{
            pdf_watermark($pdf,$outputlangs,$this->page_hauteur,$this->page_largeur,'mm',getDolGlobalString('PROPALE_DRAFT_WATERMARK'));
		}
